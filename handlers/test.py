...
# ====================================================
# DEADLINE — StateFilter(CreateTest.deadline)
# Bu handler BARCHA matnlarni ushlaydi (F.text yo'q!)
# ====================================================
...
async def get_test_deadline(message: Message, state: FSMContext):
    ...
    if message.text in ["🚫 Testni bekor qilish", "❌ Bekor qilish", "🔄 Orqaga"]:
        await state.clear()
        await message.answer("❌ Test bekor qilindi.", reply_markup=main_menu_kb())
        return
    ...
    dl_text = (
        ...
        if deadline_str
        else "♾ Deadline: <b>Yo'q (cheksiz)</b>"
    )
...
async def get_test_code(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"{dl_info}\n\n"
        f"✏️ Javoblarni yuboring. Format: <b>ABCDDCBA...</b>",
        reply_markup=back_kb(),
        parse_mode="HTML"
    )
...
async def check_answers(message: Message, state: FSMContext, bot: Bot):
    ...
    if len(user_answers) < total:
        await message.answer(
            f"❌ {len(user_answers)} ta javob keldi, {total} ta kerak!\n"
            f"Qayta to'liq yuboring."
        )
        return
